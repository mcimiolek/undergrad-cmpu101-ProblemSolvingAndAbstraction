...
; produces the base where the blocks have fallen previously
...
(define (game-over tet)
  (cond
    [(not (empty? (tetris-landscape tet)))
     (cond
       [( <= (block-y (first (tetris-landscape tet))) 50) true]
       [else false])]
     [else false]))

(check-expect (game-over (make-tetris (make-block 100 100) (list (make-block 500 100)))) false)
(check-expect (game-over (make-tetris (make-block 100 100) '())) false)
(check-expect (game-over (make-tetris (make-block 100 100) (list (make-block 50 100)))) false)